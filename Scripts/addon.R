...
allEvents=timeSerToEvents(timeSer=rainfall[,c(1,10)],thres_cond1=10,thres_cond2=20,
                          thres_cond3=5,plot=TRUE)
tsInt=rainfall.zoo[,-9]
...
for (i in int.events) {
  ...
  bbb=rollapply(aaa.new,3,mean,by=3) # for 15 min
  ...
  write.csv(as.data.frame(aaa),paste0(i,"_",5,".csv"))
  write.csv(as.data.frame(bbb),paste0(i,"_",15,".csv"))
  write.csv(as.data.frame(ccc),paste0(i,"_",30,".csv"))
...
for (i in int.events) {
  ...
  write.csv(as.data.frame(ddd),paste0(i,"_",2,".csv"))
